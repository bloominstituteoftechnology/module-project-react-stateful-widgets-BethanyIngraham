...
export default function Programmers() {
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: selectedProgrammer ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setSelectedProgrammer(dev.id) /* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
